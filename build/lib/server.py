...
def _board_bin() -> str:
    """BOARD_BIN env > this checkout's bin/board > `board` on PATH (uvx/pipx installs)."""
    explicit = os.environ.get("BOARD_BIN")
    if explicit:
        return explicit
    if os.path.exists(_DEFAULT_BOARD_BIN):
        return _DEFAULT_BOARD_BIN
    import shutil
    return shutil.which("board") or _DEFAULT_BOARD_BIN
...
# MCP resources — read-only board context. Onboarding text comes from `board brief`
# (the single source, decision 0012), so it can never drift from hook/paste channels.
RESOURCES: list[dict[str, Any]] = [
    {
        "uri": "board://onboarding",
        "name": "OpenBoard onboarding brief",
        "description": "Who you are on this board, current digest, and the work loop "
                       "(JSON from `board brief --json` — the canonical onboarding source).",
        "mimeType": "application/json",
    },
    {
        "uri": "board://digest",
        "name": "OpenBoard digest",
        "description": "Rolling board summary: agents, open tasks, recent results/decisions.",
...
def _handle_request(msg: dict[str, Any]) -> dict[str, Any] | None:
    ...
    # resources/list — static catalog; content is produced by the CLI on read
    ...
    # resources/read — onboarding comes from `board brief --json` (the single source,
    # decision 0012 mech 2); digest from `board digest`.
    if method == "resources/read":
        uri = (params or {}).get("uri", "")
        if uri == "board://onboarding":
            out, err, rc = _run_board("brief", "--json")
            if rc != 0 or out is None:
                return _error_response(req_id, -32603, f"brief failed: {err.strip() or rc}")
            return {
                "jsonrpc": "2.0",
                "id": req_id,
                "result": {"contents": [
                    {"uri": uri, "mimeType": "application/json", "text": out}
                ]},
            }
        if uri == "board://digest":
            out, err, rc = _run_board("digest")
            if rc != 0 or out is None:
                return _error_response(req_id, -32603, f"digest failed: {err.strip() or rc}")
            return {
                "jsonrpc": "2.0",
                "id": req_id,
                "result": {"contents": [
                    {"uri": uri, "mimeType": "text/markdown", "text": out}
                ]},
            }
